...
int main()
{
    ...
    Terrain paper(octaves, lucanarity, gain, HEIGHT, WIDTH, 0, 50);
    ...
    //glBufferData(GL_ARRAY_BUFFER, verticesTerrain.size() * sizeof(float), verticesTerrain.data(), GL_STATIC_DRAW);
    ...
    while (!window.shouldClose()) {
        ...
        std::cout << "cameraPos: (" << camera.cameraPos().x << "," << camera.cameraPos().y << "," << camera.cameraPos().z << ")\n";
        ...
        //const glm::mat4 mvp = camera.viewMatrix();
        int no = 0;
        int yes = 1;
        glm::vec4 overrideColor = glm::vec4(1.0f, 0.0f, 0.0f, 1.0f); 
        ...
        
        {
            ...
            glUniform4fv(1, 1, glm::value_ptr(overrideColor));

            // Render filled polygons
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);  
            glUniform1i(2, 0);  // Do not use override color
            paper.Render(); 

            //// Render wireframe overlay
            //glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
            //glUniform1i(2, 1);  // Use override color
            //paper.Render();
        }
