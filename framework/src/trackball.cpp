...
static constexpr float rotationScrollSpeedFactor = 12.0f;
...
Trackball::Trackball(Window* pWindow, float fovy, float distFromLookAt, float rotationX, float rotationY, bool active)
    : Trackball(pWindow, fovy, glm::vec3(0.0f), distFromLookAt, rotationX, rotationY, active)
{
}

Trackball::Trackball(Window* pWindow, float fovy, const glm::vec3& lookAt, float distFromLookAt, float rotationX, float rotationY, bool active)
    : m_pWindow(pWindow)
    , m_fovy(fovy)
...
{
    const bool controlPressed = m_pWindow->isKeyPressed(GLFW_KEY_LEFT_CONTROL);
    const bool shiftPressed = m_pWindow->isKeyPressed(GLFW_KEY_LEFT_SHIFT);
    ...
    if (controlPressed || shiftPressed) {
        if (shiftPressed) {
            ...
            // Translate the camera in the image plane.
            m_lookAt += float(offset.x) * zoomSpeedFactor * left(); // Mouse right => camera left
            m_lookAt += float(offset.y) * zoomSpeedFactor * up(); // Mouse up => camera down
        }
    }
    else {
        // Rotate the camera around the lookat point.
        m_rotationEulerAngles.x += glm::radians(float(offset.y) * rotationScrollSpeedFactor);
        //m_rotationEulerAngles.x = std::clamp(m_rotationEulerAngles.x - glm::radians(delta.y * rotationSpeedFactor), -glm::half_pi<float>(), +glm::half_pi<float>());
        m_rotationEulerAngles.y -= glm::radians(float(offset.x) * rotationScrollSpeedFactor);
    }
